...
std::vector<std::pair<int,
                      std::pair<double, double>>> ParticleGenerator::generateParticles(ParticleContainer &particles,
	...
	for (auto &info : infovec) {
		switch (info.type) {
			case cuboid:generateCuboid(particles, info, index);
				mapping.push_back({index, {info.epsilon, info.sigma}});
				break;
			case sphere:generateSphere(particles, info, index);
				mapping.push_back({index, {info.epsilon, info.sigma}});
				break;
		}
		index++;
		...
		for (int y = 0; y < info.N[1]; y++) {
			for (int x = 0; x < info.N[0]; x++) {
				...
				auto it = std::find_if(info.specialParticles.begin(), info.specialParticles.end(),
				                       [x, y, z](auto &s) {
				                         return x == std::get<0>(s)[0] && y == std::get<0>(s)[1]
					                         && z == std::get<0>(s)[2];
				                       });
