...
void MainUtils::get_arguments(int argc, char **argv, Config &config) {
	const std::string help =
		"Usage: ./MolSim [-x <xml_file>] [-i <input_file>] [-g <generator_input>] [-e <end_time>] [-d <delta_t>] [-w <writer>] [-c <calc>] [-b <brownian_motion_velocity_mean>] [-r]\n"
	...
	while ((opt = getopt(argc, argv, "x:hi:g:e:d:w:c:rb:m:")) != -1) {
		switch (opt) {
			case 'h':std::cout << help;
				exit(0);
			case 'x':config.xml_file = optarg;
				break;
			case 'i':config.filename.emplace_back(optarg);
				break;
			case 'g':config.generator_files.emplace_back(optarg);
				break;
			case 'e':config.end_time = std::stod(optarg);
				break;
			case 'd':config.delta_t = std::stod(optarg);
				break;
			case 'w':
			...
			case 'c':
				...
				break;
			case 'b':config.brownianMotion = true;
				config.brownianMotionMean = std::stod(optarg);
				break;
			case 'r':config.randomGen = true;
				break;
			case 'm':
				...
				break;
			case '?':std::cerr << "Unknown option: " << static_cast<char>(optopt) << "\n";
				std::cerr << help;
				break;
			default:break;
		}
...
std::unique_ptr<IOWriter> MainUtils::get_io_type(Config &config) {
	switch (config.io_type) {
		case IOWriter::xyz:return std::make_unique<outputWriter::XYZWriter>();
		case IOWriter::vtk:
		case IOWriter::unknown:
		default:return std::make_unique<outputWriter::VTKWriter>();
	}
}

std::unique_ptr<PhysicsCalc> MainUtils::get_calculator(Config &config) {
	switch (config.calc_type) {
		case PhysicsCalc::gravitation:return std::make_unique<calculator::Gravitation>();
		case PhysicsCalc::lennardJones:
		case PhysicsCalc::unknown:
		default:
			if (config.linkedCell) {
				auto c = std::make_unique<calculator::LinkedCell>(config.sigma,
				                                                  config.eps,
				                                                  config.rCut,
				                                                  config.smoothed,
				                                                  config.rl);
...
std::unique_ptr<ParticleContainer> MainUtils::get_container(Config &config) {
	if (config.linkedCell) {
		return std::make_unique<LinkedCellContainer>(config.linkedCellSize[0],
		                                             config.linkedCellSize[1],
		                                             config.linkedCellSize[2],
		                                             config.rCut,
...
std::unique_ptr<StatisticsLogger> MainUtils::get_statistics_logger(Config &config) {
	switch (config.statsType) {
		case StatisticsLogger::densityVelocityProfile:
			return std::make_unique<statistics::DensityVelocityProfile>(config.statsFile,
			                                                            config.noBins);
		case StatisticsLogger::thermodynamic:
			return std::make_unique<statistics::Thermodynamical>(config.statsFile,
			                                                     config.delta_r,
			                                                     config.maxDistance);
		default:return nullptr; //should never happen, only to silence -Wreturn-type
	}
}
...
void MainUtils::initializeParticles(ParticleContainer &particles, Config &config) {
	...
	for (auto &genFile : config.generator_files) {
		auto newTypeToSe = ParticleGenerator::generateParticles(particles,
		                                                        genFile,
		                                                        static_cast<int>(sigmaTable.size()
			                                                        + se_mapping.size()));
		se_mapping.insert(se_mapping.begin(), newTypeToSe.begin(), newTypeToSe.end());
	}
	// generate Particles directly specified in XML
	auto newTypeToSe = ParticleGenerator::generateParticles(particles,
	                                                        config.generatorInfos,
	                                                        static_cast<int>(sigmaTable.size() + se_mapping.size()));
	se_mapping.insert(se_mapping.begin(), newTypeToSe.begin(), newTypeToSe.end());
...
void MainUtils::logParticle(ParticleContainer &particles) {
	...
	for (auto &p : particles) {
		...
		auto f1 = p.getF()[1];
		auto f2 = p.getF()[2];
		spdlog::info("Moved particle ({} {} {}) with velocity ({} {} {}) and force ({} {} {})",
		             x,
		             y,
		             z,
		             v0,
...
void MainUtils::printConfig(Config &config) {
	...
	switch (config.parallelization_strategy) {
		case LinkedCellContainer::serial:s << "none" << std::endl;
			break;
		case LinkedCellContainer::primitiveX:s << "primitiveX" << std::endl;
			break;
		case LinkedCellContainer::primitiveY:s << "primitiveY" << std::endl;
			break;
		case LinkedCellContainer::primitiveZ:s << "primitiveZ" << std::endl;
			break;
		case LinkedCellContainer::subDomain:s << "subdomain" << std::endl;
			break;
	}
	...
	s << "\u001b[36m\tContainer:\u001b[0m " << (config.linkedCell ? "LinkedCellContainer" : "DirectSumContainer")
	  << std::endl;
	s << "\u001b[36m\tCalculator:\u001b[0m ";
	switch (config.calc_type) {
		case PhysicsCalc::gravitation:s << "Gravitation";
			break;
		case PhysicsCalc::lennardJones:
			s << "Lennard-Jones-Potential" << (config.linkedCell ? " (LinkedCellCalculation)" : "");
			break;
		case PhysicsCalc::unknown:
		default:s << "Lennard-Jones-Potential (Default)" << (config.linkedCell ? " (LinkedCellCalculation)" : "");
	}
	s << std::endl;
	if (config.useThermostat) {
		s << "\u001b[36m\tThermostat:\u001b[0m " << "nThermostat: " << config.nThermostat << ", Tinit: "
	...
	if (config.useStatistics) {
		s << "\u001b[36m\tStatistics ";
		switch (config.statsType) {
			case StatisticsLogger::densityVelocityProfile:s << "(DensityVelocityProfile):\u001b[0m ";
				break;
			case StatisticsLogger::thermodynamic:s << "(Thermodynamic):\u001b[0m ";
				break;
			default:s << "\u001b[0m ";
		}
	...
	s << "\u001b[36m\tWriter:\u001b[0m ";
	switch (config.io_type) {
		case IOWriter::vtk:s << "VTK-Writer";
			break;
		case IOWriter::xyz:s << "XYZ-Writer";
			break;
		case IOWriter::unknown:
		default:s << "VTK-Writer (Default)";
	}
	s << std::endl;
	...
	if (config.linkedCell) {
		for (auto &p : particles) {
			if (p.getX()[0] < 0 || p.getX()[0] > config.linkedCellSize[0] ||
				...
				std::cout << "\u001b[31mParticle at " << p.getX() << " out of linked cell domain bounds ("
				          << config.linkedCellSize << ")\u001b[0m" << std::endl;
				abort = true;
				break;
			}
		}
