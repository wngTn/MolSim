...
XMLReader::XMLInfo XMLReader::readFile(const std::string &s) {
	...
	switch (sim->calculator().type()) {
		case calculatortype_t::smoothed_lennardjones:info.smoothed = true;
			info.rl = sim->calculator().rl().get();
			[[fallthrough]];
		case calculatortype_t::lennardjones:info.epsilon = sim->calculator().epsilon().get();
			...
			if (sim->calculator().gravityFactor().present()) {
				info.gravityFactor = {sim->calculator().gravityFactor()->x(), sim->calculator().gravityFactor()->y(),
				                      sim->calculator().gravityFactor()->z()};
			} else {
			...
			break;
		case calculatortype_t::gravitation:info.calculatorType = PhysicsCalc::gravitation;
			break;
	}
	...
	switch (sim->outputWriter()) {
		case outputwriter_t::vtk:info.outputWriterType = IOWriter::vtk;
			break;
		case outputwriter_t::xyz:info.outputWriterType = IOWriter::xyz;
			break;
	}
	...
	if (sim->statistics().present()) {
		...
		switch (sim->statistics()->type()) {
			case statistics_type_t::thermodynamical:info.statsType = StatisticsLogger::thermodynamic;
				info.noBins = 0;
				info.delta_r = sim->statistics()->deltaR().get();
				info.maxDistance = sim->statistics()->maxDistance().get();
				break;
			case statistics_type_t::densityVelocity:info.statsType = StatisticsLogger::densityVelocityProfile;
	...
	if (sim->parallelization().present()) {
		switch (sim->parallelization().get()) {
			case parallel_t::subdomain:info.parallelization_strat = LinkedCellContainer::subDomain;
				break;
			case parallel_t::primitiveX:info.parallelization_strat = LinkedCellContainer::primitiveX;
				break;
			case parallel_t::primitiveY:info.parallelization_strat = LinkedCellContainer::primitiveY;
				break;
			case parallel_t::primitiveZ:info.parallelization_strat = LinkedCellContainer::primitiveZ;
				break;
			case parallel_t::none:info.parallelization_strat = LinkedCellContainer::serial;
				break;
		}
	}
...
void XMLReader::initializeBorderType(int index,
                                     border_single_t &type,
                                     std::array<LinkedCellContainer::Border, 6> &boundaryConds) {
	switch (type) {
		case border_single_t::periodic:boundaryConds[index] = LinkedCellContainer::periodic;
			break;
		case border_single_t::reflective:boundaryConds[index] = LinkedCellContainer::reflective;
			break;
		case border_single_t::outflow:boundaryConds[index] = LinkedCellContainer::outflow;
			break;
	}
}
...
void XMLReader::insertGeneratorInfo(std::vector<ParticleGenerator::ShapeInfo> &genInfos, generator_info_t &info) {
	...
	switch (info.type()) {
		case geometric_t::cuboid:shapeInfo.N = {info.n1().get(), info.n2().get(), info.n3().get()};
			shapeInfo.type = ParticleGenerator::cuboid;
			break;
		case geometric_t::sphere:shapeInfo.radius = info.radius().get();
	...
	if (info.behaviour().present()) {
		switch (info.behaviour().get()) {
			case behaviour_t::membrane:shapeInfo.behaviour = ParticleGenerator::membrane;
				break;
			case behaviour_t::immovable:shapeInfo.behaviour = ParticleGenerator::immovable;
				break;
			case behaviour_t::normal:
			default:shapeInfo.behaviour = ParticleGenerator::normal;
				break;
		}
	}
	...
	if (info.baseForce().present()) {
		shapeInfo.baseForce =
			std::array<double, 3>{info.baseForce()->x(), info.baseForce()->y(), info.baseForce()->z()};
	}
